...
async function searchImagesByStatement(statement, topK = 10) {
  try {
    ...
    if (result.length > 0) {
      ...
      const SIMILARITY_THRESHOLD = 0.35; // Only return results with >35% similarity
...
// 🔍 Helper function to expand search queries with focused synonyms
function expandSearchQuery(query) {
  const queryLower = query.toLowerCase();
  
  // More focused expansion - only add highly relevant synonyms
  if (queryLower.includes('house') || queryLower.includes('home') || queryLower.includes('building')) {
    return `${query} house home residence dwelling building villa mansion property architecture`;
  }
  if (queryLower.includes('car') || queryLower.includes('vehicle') || queryLower.includes('automobile')) {
    return `${query} car vehicle automobile sedan coupe sports transportation wheels tires tyres driving`;
  }
  if (queryLower.includes('pool') || queryLower.includes('swimming')) {
    return `${query} pool swimming water feature recreation`;
  }
  if (queryLower.includes('person') || queryLower.includes('people') || queryLower.includes('human')) {
    return `${query} person people human individual man woman child`;
  }
  
  // For other queries, just return original
  return query;
}
...
Keep it KEYWORD-DENSE but natural for embedding similarity.`;
            ...
            // Create focused semantic text - use GPT description as primary (it's already optimized)
            // Add AWS labels as secondary support
            semanticText = `${gptDescription} ${allFeatures.join(' ')}`;
